 ...
 useEffect(()=>{
  ...
  const send=(schedule=true)=>{
   if(worker&&running){
    ...
    const held=(code:string)=>device==='keyboard'&&(keys.has(code)||touch.current.has(code));
  ...
  const keydown=(e:KeyboardEvent)=>{const key=code(e);if(keyboard[key]||['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(key)){e.preventDefault();keys.add(key);send(false);}};
