...
def build(ident):
    ...
    fitted['image']=fitted['image'].resize((256,256),Image.Resampling.LANCZOS)
    profile['browser_skinning']=True
    stats=replace_costume(archive,fitted,skel,profile)
    target=out/'browser';target.mkdir(exist_ok=True);(target/filename).write_bytes(archive.serialize())
